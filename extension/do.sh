...
ssr_generate_jsdeps() {
  ...
  $PYTHON_CMD lib/closure-library/closure/bin/build/depswriter.py \
    alerts_url.js background_url.js background_page_url.js \
...
ssr_build_extension() {
  ...
  jscompile_ssr+=" --js='./alerts_url.js'"
  jscompile_ssr+=" --js='./background_url.js'"
  jscompile_ssr+=" --js='./background_page_url.js'"
